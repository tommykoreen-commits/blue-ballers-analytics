# Blue Ballers Analytics — Streamlit Dashboard Launcher (Colab)
#
# Paste this entire file into a single Colab code cell and run it.
# Requires blue_ballers_sync.py/.ipynb to have already been run at least
# once (needs blue_ballers.db to exist in your Google Drive).
...
APP_PATH = f"{DATA_DIR}/dashboard_app.py"

if not os.path.exists(DB_PATH):
    raise SystemExit(
        f"No database found at {DB_PATH} — run the Sleeper sync notebook/script first."
    )

APP_SOURCE = r'''
# Blue Ballers Analytics — Streamlit Dashboard
#
# Reads the SQLite database built by blue_ballers_sync.py/.ipynb. Run with:
#   streamlit run dashboard_app.py
# (In Colab, use blue_ballers_dashboard.py to launch this automatically.)
...
DB_PATH = "/content/drive/MyDrive/BlueBallersAnalytics/blue_ballers.db"
...
print("Wrote dashboard app to:", APP_PATH)

def is_up():
    try:
        return requests.get("http://localhost:8501", timeout=2).status_code == 200
    except requests.exceptions.RequestException:
        return False


# Always restart with the freshest dashboard_app.py — an already-running Streamlit process
# keeps serving whatever code was loaded when it started, so reusing it silently would mask
# any edit made since (e.g. a bug fix) instead of picking it up.
def port_in_use(port):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        return s.connect_ex(("localhost", port)) == 0


# Kill whatever's actually bound to port 8501 first — more reliable than pattern-matching
# the command line below, since a stale/zombie process or a path-formatting mismatch can
# make `pkill -f` silently match nothing, leaving the OLD server up and serving stale code
# while this cell reports success.
try:
    import psutil
    for conn in psutil.net_connections(kind="inet"):
        if conn.laddr and conn.laddr.port == 8501 and conn.pid:
            try:
...
subprocess.run(["pkill", "-9", "-f", "streamlit run"], check=False)

wait_deadline = time.time() + 15
while port_in_use(8501) and time.time() < wait_deadline:
    time.sleep(0.5)

if port_in_use(8501):
    print("Warning: port 8501 is still occupied after killing old process(es) — "
          "the dashboard below may still be serving STALE code. If it looks unchanged, "
          "restart the Colab runtime (Runtime > Restart session) and re-run this cell.")

# Streamlit resolves .streamlit/config.toml relative to the process's working
# directory, not the script's own directory — so this must live in DATA_DIR
# and the subprocess must be launched with cwd=DATA_DIR (below) for the theme
# to actually take effect.
STREAMLIT_CONFIG_DIR = f"{DATA_DIR}/.streamlit"
os.makedirs(STREAMLIT_CONFIG_DIR, exist_ok=True)
with open(f"{STREAMLIT_CONFIG_DIR}/config.toml", "w", encoding="utf-8") as f:
    f.write(
        '[theme]\n'
        'base="dark"\n'
        'primaryColor="#3B82F6"\n'
        'backgroundColor="#0B1220"\n'
        'secondaryBackgroundColor="#111C2E"\n'
        'textColor="#E5E7EB"\n'
        'font="sans serif"\n'
    )
...
proc = subprocess.Popen(
    ...
    stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, cwd=DATA_DIR,
)
...
if not started:
    if proc is not None and proc.poll() is not None:
        print("Streamlit exited early. Process output:")
        print(proc.stdout.read())
    else:
        print("Streamlit hasn't responded yet but the process is still running — wait a few "
              "seconds and re-run this cell.")
else:
    # Colab's built-in port-serving (serve_kernel_port_as_window/_iframe) proxies plain HTTP
    # fine but doesn't reliably forward the WebSocket connection Streamlit needs to render —
    # that's why the embedded view got stuck loading. A Cloudflare quick tunnel proxies
    # WebSockets properly and needs no signup.
    ...
    if "tunnel_url" in globals() and tunnel_url and tunnel_is_alive(tunnel_url):
        print("Reusing existing tunnel:", tunnel_url)
    else:
        if "tunnel_url" in globals() and tunnel_url:
            print("Previous tunnel is no longer responding — starting a fresh one.")
        tunnel_proc = subprocess.Popen(
            [CLOUDFLARED_PATH, "tunnel", "--url", "http://localhost:8501", "--no-autoupdate"],
        ...
        if tunnel_url:
            print("\nDashboard is live at:", tunnel_url)
        else:
            print("Could not detect the tunnel URL within 30s. Recent cloudflared output:")
            print("".join(recent_lines[-20:]))
